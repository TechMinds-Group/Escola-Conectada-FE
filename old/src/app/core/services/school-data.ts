...
export class SchoolDataService {
  ...
  // HTTP Methods
  async loadSubjects() {
    const data = await firstValueFrom(this.http.get<any[]>(`${this.apiUrl}/Materias`));
    this.subjects.set(data.map((item) => this.mapToSubject(item)));
  }
  ...
  async loadMatrices() {
    const data = await firstValueFrom(this.http.get<any[]>(`${this.apiUrl}/MatrizesEscolares`));
    this.schoolMatrices.set(data.map((item) => this.mapToSchoolMatrix(item)));
  }

  async getMatrixById(id: string): Promise<SchoolMatrix> {
    const item = await firstValueFrom(this.http.get<any>(`${this.apiUrl}/MatrizesEscolares/${id}`));
  ...
  async loadTimeGrids() {
    const data = await firstValueFrom(this.http.get<any[]>(`${this.apiUrl}/GradesHoraria`));
    this.schoolTimeGrids.set(data.map((item) => this.mapToSchoolTimeGrid(item)));
  }
  ...
  private mapToSubject(item: any): Subject {
    return {
      ...
      code: undefined, // Code is now used for Axis, so we don't expose it as code
      axis: (item.codigo as ThematicAxis) || ThematicAxis.Outros, // Map Codigo -> Axis
